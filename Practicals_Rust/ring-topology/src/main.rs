...
// ring: Anirudha -> Bubbu -> Cairi -> Anirudha (wraps around).
//
// Build: rustc p2p_ring_topology.rs -o ring
// Run (3 terminals, same roster string, only <my_name> differs):
//   ./ring Anirudha Anirudha:127.0.0.1:9000,Bubbu:127.0.0.1:9001,Cairi:127.0.0.1:9002
//   ./ring Bubbu   Anirudha:127.0.0.1:9000,Bubbu:127.0.0.1:9001,Cairi:127.0.0.1:9002
//   ./ring Cairi Anirudha:127.0.0.1:9000,Bubbu:127.0.0.1:9001,Cairi:127.0.0.1:9002
//
...
fn main() {
    ...
    println!(
        "[{}] Listening on {} (for predecessor: {})",
        my_name, listen_addr, pred_name
    );
    ...
    println!(
        "[{}] Type a message + Enter to send it around the ring. Type 'exit' to quit.\n",
        my_name
    );
    let stdin = io::stdin();
...
fn handle_ring_message(my_name: &str, line: &str, succ_writer: &Arc<Mutex<Option<TcpStream>>>) {
    ...
    if origin == my_name {
        println!(
            "[{}] Message completed the ring after {} hop(s): \"{}\"",
            my_name, hop, text
        );
        return;
    }
    ...
    println!(
        "[{}] Relaying message from {} (hop {}): \"{}\"",
        my_name, origin, hop, text
    );
